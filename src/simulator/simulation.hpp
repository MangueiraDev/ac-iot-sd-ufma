...
inline void update_state(RoomState& s) {
    ...
    if (s.modo_ac == "ativo") {
        if (!s.presenca) {
            ...
            if (ac_ok && (s.temp > s.setpoint_ac + 1.0 || s.umidade > s.setpoint_umidade + 6.0))
                s.status_ac = "ligado";
            else if (s.temp < s.setpoint_ac - 1.5 && s.umidade <= s.setpoint_umidade + 2.0)
                s.status_ac = "desligado";
        }
    }

    // Atualização física da temperatura
    double tgt_t = (s.status_ac == "ligado")
